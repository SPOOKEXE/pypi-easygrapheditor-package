...
const scope=typeof parentElement==='undefined'?document:parentElement;let root=scope.querySelector({json.dumps('#' + instance)});if(!root)return;const fresh=root.cloneNode(true);root.replaceWith(fresh);root=fresh;const g=JSON.parse(root.dataset.graph),svg=root.querySelector('svg'),world=root.querySelector('.world'),status=root.querySelector('.status'),dock=root.querySelector('.ege-dock-body');let dockTab='inspector';
...
function fit(){{if(!g.nodes.length)return;const r=svg.getBoundingClientRect(),minX=Math.min(...g.nodes.map(n=>n.x)),minY=Math.min(...g.nodes.map(n=>n.y)),maxX=Math.max(...g.nodes.map(n=>n.x+nodeWidth(n))),maxY=Math.max(...g.nodes.map(n=>n.y+Math.max(74,68+Math.max(n.inputs.length,n.outputs.length)*16)));const z=Math.max(.35,Math.min(2.5,(r.width-60)/Math.max(1,maxX-minX),(r.height-60)/Math.max(1,maxY-minY)));view={{x:(r.width-(maxX-minX)*z)/2-minX*z,y:(r.height-(maxY-minY)*z)/2-minY*z,zoom:z}};tx();}}
...
svg.addEventListener('pointerup',e=>{{const port=e.target.closest('.port');if(wire&&port&&wire.dir==='out'&&port.dataset.dir==='in')emit('connect',{{from_node:wire.from_node,from_port:wire.from_port,to_node:port.dataset.node,to_port:port.dataset.port}});if(drag)emit('positions',{{positions:Object.fromEntries(g.nodes.map(n=>[n.id,[n.x,n.y]])),viewport:view}});if(marquee){{const a=local(marquee.p),b=local(marquee.q),x0=Math.min(a.x,b.x),x1=Math.max(a.x,b.x),y0=Math.min(a.y,b.y),y1=Math.max(a.y,b.y);emit('marquee',{{nodes:g.nodes.filter(n=>n.x>=x0&&n.x<=x1&&n.y>=y0&&n.y<=y1).map(n=>n.id)}});}}if(pan)emit('viewport',view);drag=pan=wire=marquee=null;}});
...
renderDock=(()=>{{const original=renderDock;return()=>{{if(dockTab==='inspector'){{renderInspectorDock();return;}}original();}};}})();renderDock();
}})();</script></div>'''
...
def editor_canvas_js(state: EditorState, *, editable: bool = True, outputs: dict[str, dict[str, Any]] | None = None) -> str:
    ...
    initializer = markup.split("<script>", 1)[1].rsplit("</script>", 1)[0]
    return ("const initializeCanvas = () => {" + initializer + "}; "
            "initializeCanvas(); if (typeof watch === 'function') "
            "watch('value', () => requestAnimationFrame(initializeCanvas));")
