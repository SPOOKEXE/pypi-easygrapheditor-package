...
#   ./scripts/run-demo.sh --demo noiseterrain --ui --backend pygame|gradio|streamlit
...
declare -A DEMOS=(
  ...
  [terrain]="terrain_demo.py|Headless terrain graph with cache verification"
...
usage() {
  cat <<EOF
Usage: run-demo.sh --demo NAME [--ui] [--backend BACKEND] [--mode MODE] [-- extra args]
...
Without --ui, the demo runs headless and prints results.

Backends (--backend, used with --ui): pygame gradio streamlit
...
Examples:
  ...
  ./scripts/run-demo.sh --demo noiseterrain --ui --backend pygame
  ./scripts/run-demo.sh --demo stagedarith --ui --backend gradio --mode read-only
  ./scripts/run-demo.sh --list
EOF
}
...
if [[ " pygame gradio streamlit " != *" $BACKEND "* ]]; then
  echo "error: unknown --backend '$BACKEND' (choose: pygame gradio streamlit)" >&2; exit 2
fi
if [[ "$MODE" != "live" && "$MODE" != "read-only" ]]; then
  echo "error: unknown --mode '$MODE' (choose: live read-only)" >&2; exit 2
fi
...
if [[ "$UI" -eq 0 ]]; then
  ...
  exec uv run python "$EXAMPLES/$SCRIPT" --ui --backend "$BACKEND" --mode "$MODE" "${EXTRA[@]}"
fi
